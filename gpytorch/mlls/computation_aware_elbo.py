...
class ComputationAwareELBO(MarginalLogLikelihood):
    ...
    def forward(self, outputs: torch.Tensor, targets: torch.Tensor, **kwargs):
        ...
        prior_evaluated_at_train_inputs = outputs[
            0:
            num_train_data]  # Training data size might not exactly equal NNZ * PROJ_DIM
        ...
        if settings.debug.on():
            ...
            if (not torch.equal(
                    train_inputs,
                    outputs.lazy_covariance_matrix.x1[0:num_train_data])) or (
                        not torch.equal(train_targets,
                                        targets[0:num_train_data])):
                raise RuntimeError(
        ...
        K_lazy = kernel_forward_fn(
            train_inputs.div(lengthscale).view(self.model.projection_dim,
                                               self.model.num_non_zero,
                                               train_inputs.shape[-1]),
            train_inputs.div(lengthscale).view(self.model.projection_dim, 1,
                                               self.model.num_non_zero,
                                               train_inputs.shape[-1]),
        )

        # Compute S'K in block shape (PROJ_DIM, PROJ_DIM, NNZ)
        StK_block_shape = (K_lazy @ self.model.actions_op.blocks.view(
            self.model.projection_dim, 1, self.model.num_non_zero,
            1)).squeeze(-1)
        covar_x_batch_X_train_actions = (StK_block_shape.view(
            self.model.projection_dim, self.model.projection_dim *
            self.model.num_non_zero).mul(outputscale).mT)

        # Projected Gramians S'KS and S'(K + noise)S
        gram_SKS = (StK_block_shape *
                    self.model.actions_op.blocks).sum(-1).mul(outputscale)
        ...
        actions_targets = self.model.actions_op._matmul(
            torch.atleast_2d(train_targets -
                             prior_evaluated_at_train_inputs.mean).mT).squeeze(
                                 -1)
        ...
        f_pred_mean_batch = prior_evaluated_at_train_inputs.mean + covar_x_batch_X_train_actions @ torch.atleast_1d(
            compressed_repr_weights).to(dtype=targets.dtype)
        ...
        f_pred_var_batch = torch.sum(prior_evaluated_at_train_inputs.variance
        ...
        kl_prior_term = 0.5 * (
            ...
            torch.log(StrS_diag.to(dtype=torch.float64).sum()) - torch.trace(
