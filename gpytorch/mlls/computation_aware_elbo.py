...
class ComputationAwareELBO(MarginalLogLikelihood):
    ...
    def forward(self, outputs: torch.Tensor, targets: torch.Tensor, *params,
        ...
        K_lazy = self.model.covar_module(
            train_inputs.view(self.model.projection_dim,
                              self.model.num_non_zero, train_inputs.shape[-1]),
            train_inputs.view(self.model.projection_dim, 1,
                              self.model.num_non_zero, train_inputs.shape[-1]),
        )
        ...
        covar_x_batch_X_train_actions = StK_block_shape.view(
            ...
            self.model.projection_dim * self.model.num_non_zero).mT
        ...
        kl_prior_term = 0.5 * (
            ...
            torch.log(StrS_diag.to(
                dtype=torch.float64)).sum()  # NOTE: Assumes orthogonal actions
            - torch.trace(
