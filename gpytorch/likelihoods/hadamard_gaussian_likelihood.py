...
def _check_task_indices(task_idcs: torch.Tensor,
                        base_shape: torch.Size) -> None:
    r"""
    ...
    """
    is_int_dtype = task_idcs.dtype in (torch.uint8, torch.int8, torch.int16,
                                       torch.int32, torch.int64)
    task_batch_shape = task_idcs.shape[:-1]
    sub_base_shape = base_shape[-len(task_batch_shape):]
    shape_matches = task_batch_shape == sub_base_shape and task_idcs.shape[
        -1] == 1
    if not (torch.is_tensor(task_idcs) and shape_matches and is_int_dtype):
        raise ValueError(
            f"Expected task indexes must be a tensor of shape {(*sub_base_shape, 1)}, with integer dtype."
        )
    ...
    def _shaped_noise_covar(self, base_shape: torch.Size, *params: Any,
        ...
        if task_idcs.shape[-1] > 1:
            if self.task_feature_index is None:
                raise ValueError(
                    "Task indices must be a single dimension if task_feature_index is not provided."
                )
        ...
        noise_base_covar_matrix = self.noise_covar(task_idcs,
                                                   *params[1:],
                                                   shape=base_shape,
                                                   **kwargs)
        if self.num_tasks > 1:
            ...
            noise_base_covar_matrix = noise_base_covar_matrix.squeeze(
                -4)  # (*batch_shape, num_tasks, num_data, num_data)
        all_tasks = torch.arange(self.num_tasks,
                                 device=task_idcs.device).unsqueeze(
                                     -1)  # (num_tasks, 1)
