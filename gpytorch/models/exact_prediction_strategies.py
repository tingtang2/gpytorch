...
class DefaultPredictionStrategy(object):
    ...
    def __init__(self,
        ...
        try:
            train_labels = train_labels.reshape(
                *train_labels.shape[:-len(self.train_shape)],
                self._train_shape.numel())
        except RuntimeError:
            raise RuntimeError(
                ...
                + "The shape of the train targets is {0}, ".format(
                    train_labels.shape) +
        ...
        if inv_root is not None:
            add_to_cache(self.lik_train_train_covar, "root_inv_decomposition",
    ...
    def get_fantasy_strategy(self, inputs, targets, full_inputs, full_targets,
        ...
        if isinstance(full_output, MultitaskMultivariateNormal):
            full_mean = full_mean.view(*target_batch_shape, -1,
                                       num_tasks).contiguous()
    ...
    def _mean_cache(self, nan_policy: str) -> Tensor:
        ...
        if nan_policy == "ignore":
            ...
            kernel = MaskedLinearOperator(train_train_covar.evaluate_kernel(),
                                          observed.reshape(-1),
                                          observed.reshape(-1))
            mean_cache[..., observed] = kernel.solve(
                train_labels_offset[..., observed, :]).squeeze(-1)
        else:  # 'fill'
            ...
            mean_cache[
                missing] = torch.nan  # Ensure that nobody expects these values to be valid.
        if settings.detach_test_caches.on():
    ...
    def exact_prediction(self, joint_mean, joint_covar):
        ...
        if joint_covar.size(-1) <= settings.max_eager_kernel_size.value():
            ...
            test_test_covar = joint_covar[..., self.num_train:,
                                          self.num_train:]
    ...
    def exact_predictive_mean(self, test_mean: Tensor,
                              test_train_covar: LinearOperator) -> Tensor:
        """
        ...
        if nan_policy == "ignore":
            ...
            full_mask = torch.ones(test_mean.shape[-1],
                                   dtype=torch.bool,
                                   device=test_mean.device)
            test_train_covar = MaskedLinearOperator(
                to_linear_operator(test_train_covar), full_mask,
                observed.reshape(-1))
            res = (test_train_covar
                   @ mean_cache[..., observed].unsqueeze(-1)).squeeze(-1)
        else:  # 'fill'
    ...
    def exact_predictive_covar(
            self, test_test_covar: LinearOperator,
            test_train_covar: LinearOperator) -> LinearOperator:
        """
        Computes the posterior predictive covariance of a GP
...
class InterpolatedPredictionStrategy(DefaultPredictionStrategy):
    ...
    def __init__(self,
        ...
        super().__init__(train_inputs, train_prior_dist, train_labels,
                         likelihood)
    ...
    def _exact_predictive_covar_inv_quad_form_cache(self,
        ...
        res = base_linear_op.matmul(
            left_t_interp(train_interp_indices, train_interp_values,
    ...
    def get_fantasy_strategy(self, inputs, targets, full_inputs, full_targets,
        ...
        # Evaluate fant x train and fant x fant covariance matrices, leave train x train unevaluated.
        fant_fant_covar = full_covar[..., num_train:,
                                     num_train:].evaluate_kernel()
        fant_mean = full_mean[..., num_train:]
        ...
        fant_noise = fant_likelihood.noise_covar(
            fant_wmat.transpose(-1, -2) if len(fant_wmat.shape) >
            2 else fant_wmat)
        fant_root_vector = fant_noise.sqrt_inv_matmul(
            fant_wmat.transpose(-1, -2)).transpose(-1, -2)

        new_wmat = self.interp_inner_prod.add_low_rank(
            fant_root_vector.to_dense())
        ...
        add_to_cache(fant_strat, "interp_response_cache",
                     new_interp_response_cache)
        return fant_strat
    ...
    def prepare_dense_wmat(self, covar=None):
        ...
        wmat = covar._sparse_left_interp_t(
            covar.left_interp_indices, covar.left_interp_values).to_dense()
    ...
    def interp_inner_prod(self):
        ...
        interp_inner_prod = wmat.matmul(
            noise_term.solve(wmat.transpose(-1, -2)))
    ...
    def mean_cache(self):
        ...
        train_train_covar_inv_labels = train_train_covar_with_noise.solve(
            mean_diff)
    ...
    def fantasy_mean_cache(self):
        ...
        interp_inner_prod_root = self.interp_inner_prod.root_decomposition(
            method="cholesky").root
        # M = KL
        inducing_compression_matrix = inducing_covar.matmul(
            interp_inner_prod_root)

        # Q = L'KL + 1
        current_qmatrix = interp_inner_prod_root.transpose(
            -1, -2).matmul(inducing_compression_matrix).add_jitter(1.0)

        # m = K_UU WD^{-1}(y - \mu)
        inducing_covar_response = inducing_covar.matmul(
            self.interp_response_cache)

        # L' m
        root_space_projection = interp_inner_prod_root.transpose(
            -1, -2).matmul(inducing_covar_response)
        # Q^{-1} (L' m)
    ...
    def fantasy_covar_cache(self):
        ...
        if settings.fast_pred_var.on():
            ...
            inner_cache = RootLinearOperator(
                inducing_compression_matrix.matmul(
                    qmat_inv_root.root.to_dense()))
        else:
            inner_cache = inducing_compression_matrix.matmul(
                current_qmatrix.solve(
                    inducing_compression_matrix.transpose(-1, -2)))
    ...
    def covar_cache(self):
        ...
        vector_indices = torch.randperm(num_inducing).type_as(
            train_interp_indices)
        probe_vector_indices = vector_indices[:num_probe_vectors]
        test_vector_indices = vector_indices[num_probe_vectors:2 *
                                             num_probe_vectors]
        ...
        probe_interp_values = torch.ones(num_probe_vectors,
                                         1,
                                         dtype=dtype,
                                         device=device)
        ...
        probe_vectors = InterpolatedLinearOperator(
            ...
            train_interp_indices.expand(*batch_shape,
                                        *train_interp_indices.shape[-2:]),
            train_interp_values.expand(*batch_shape,
                                       *train_interp_values.shape[-2:]),
            probe_interp_indices.expand(*batch_shape,
                                        *probe_interp_indices.shape[-2:]),
            probe_interp_values.expand(*batch_shape,
                                       *probe_interp_values.shape[-2:]),
        ).to_dense()
        ...
        train_train_covar_inv_root = train_train_covar_plus_noise.root_inv_decomposition(
            initial_vectors=probe_vectors, test_vectors=test_vectors).root
        ...
        # Precomputed factor
        if settings.fast_pred_samples.on():
            inside = train_train_covar.base_linear_op + RootLinearOperator(
                root).mul(-1)
    ...
    def exact_prediction(self, joint_mean, joint_covar):
        ...
        test_mean = joint_mean[..., self.num_train:]
        test_test_covar = joint_covar[..., self.num_train:,
                                      self.num_train:].evaluate_kernel()
        test_train_covar = joint_covar[
            ..., self.num_train:, :self.num_train].evaluate_kernel()
    ...
    def exact_predictive_covar(self, test_test_covar, test_train_covar):
        ...
        if self.uses_wiski:
            ...
            precomputed_cache = self.covar_cache
            fps = settings.fast_pred_samples.on()
            if (fps and precomputed_cache[0]
                    is None) or (not fps and precomputed_cache[1] is None):
...
class RFFPredictionStrategy(DefaultPredictionStrategy):
    ...
    def covar_cache(self):
        ...
        train_train_covar = self.lik_train_train_covar
        inner_term = (torch.eye(train_factor.size(-1),
                                dtype=train_factor.dtype,
                                device=train_factor.device) -
                      (train_factor.transpose(-1, -2)
                       @ train_train_covar.solve(train_factor)) * constant)
    ...
    def exact_predictive_covar(self, test_test_covar, test_train_covar):
        ...
        if isinstance(test_test_covar, ConstantMulLinearOperator):
            ...
            constant = torch.tensor(1.0,
                                    dtype=test_test_covar.dtype,
                                    device=test_test_covar.device)
...
class LinearPredictionStrategy(DefaultPredictionStrategy):
    ...
    def exact_prediction(
            self, joint_mean: Tensor,
            joint_covar: LinearOperator) -> tuple[Tensor, LinearOperator]:
        """
        ...
        posterior_predictive_mean = (
            features @ mean_cache).squeeze(-1) + test_mean
        posterior_predictive_covar = RootLinearOperator(
            torch.linalg.solve_triangular(covar_cache,
                                          features.mT,
                                          upper=False).mT)
    ...
    def mean_covar_cache(self) -> tuple[Tensor, Tensor]:
        ...
        train_factor = lt.root.to_dense()
        noise = self.lik_train_train_covar._diag_tensor.diagonal(dim1=-1,
                                                                 dim2=-2)
        ...
        eye = torch.eye(d,
                        dtype=scatter_matrix.dtype,
                        device=scatter_matrix.device)
        ...
        d_sqrt_inv_train_y_offset = ((self.train_labels - train_mean) /
                                     noise).unsqueeze(-1)
        mean_cache = torch.div(
            torch.cholesky_solve(train_factor.mT @ d_sqrt_inv_train_y_offset,
                                 inv_scatter_chol,
                                 upper=False),
            constant.sqrt(),
        )
    ...
    def get_fantasy_strategy(self, inputs, targets, full_inputs, full_targets,
        ...
        res = self.__class__(
            train_inputs=full_inputs,
            train_prior_dist=self.train_prior_dist.__class__(
                full_output.mean, full_output.lazy_covariance_matrix),
            train_labels=full_targets,
    ...
    def exact_predictive_mean(self, *args, **kwargs):
        ...
        raise RuntimeError(
            "This method should not be called (use exact_prediction instead)!")
...
class SGPRPredictionStrategy(DefaultPredictionStrategy):
    ...
    def covar_cache(self):
        ...
        root = train_train_covar._linear_op.root_decomposition().root.to_dense(
        )  # R
        ...
        chol_factor = to_linear_operator(
            root.transpose(-1, -2) @ (inv_diag @ root)).add_diagonal(
                ones)  # (I + \sigma^{-2} R^T R)^{-1}
        woodbury_term = inv_diag @ torch.linalg.solve_triangular(
            chol_factor.cholesky().to_dense(),
            root.transpose(-1, -2),
        ...
        inverse = AddedDiagLinearOperator(
            inv_diag,
            MatmulLinearOperator(-woodbury_term,
                                 woodbury_term.transpose(-1, -2)))
        # \sigma^{-2} ( I - \sigma^{-2} R (I + \sigma^{-2} R^T R)^{-1} R^T  )
    ...
    def exact_prediction(self, joint_mean, joint_covar):
        ...
        if isinstance(test_test_covar,
                      LazyEvaluatedKernelTensor) and isinstance(
