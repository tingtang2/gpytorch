...
class _BlockDiagonalSparseLinearOperator(operators.LinearOperator):
    ...
    ) -> Union[Float[torch.Tensor, "... M C"], Float[torch.Tensor,
                                                     "... M"]]:  # noqa F722
        # Workarounds for (Added)DiagLinearOperator
        if isinstance(rhs, operators.AddedDiagLinearOperator):
            return self._matmul(rhs._linear_op) + self._matmul(
                rhs._diag_tensor)
    ...
    def to_dense(self: operators.LinearOperator) -> Tensor:
        ...
        return torch.zeros(
            ...
            device=self.blocks.device).scatter_(src=self.blocks,
                                                index=self.non_zero_idcs,
                                                dim=1)
    ...
    def __call__(self, x: torch.Tensor) -> MultivariateNormal:
        ...
        if self.training:
            ...
            compressed_repr_weights = (torch.cholesky_solve(
                ...
                cholfac_gram_SKhatS,
                upper=False).squeeze(-1).to(self.train_inputs[0].dtype))

            # (Combined) posterior mean and covariance evaluated at the test point(s)
            mean = self.mean_module(
                x) + covar_x_train_actions @ compressed_repr_weights
